...
Once NPM and NodeJS are installed, install the project's dependencies via `npm install`.
...
This package may be installed globally and will provide a new script/command `shipping-sort`. You may install this package globally via `npm install -g .`. To call the new global package script, you can do so via `shipping-sort [file]`.
